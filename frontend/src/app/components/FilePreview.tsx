    ...
    <div className="card-elevated animate-scale-in">
      <div className="px-5 py-3.5 border-b border-[var(--border)] flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-[var(--foreground)]">File Preview</h3>
            <p className="text-[11px] text-[var(--muted-foreground)]">{filename} &middot; {format.toUpperCase()} &middot; {rowCount} rows &middot; {columns.length} cols</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={onClose} className="text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors cursor-pointer">
        ...
        <div className="overflow-x-auto max-h-72 overflow-y-auto">
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-[var(--card)] z-10">
              <tr>
                <th className="text-left px-3 py-2 font-bold text-[var(--muted-foreground)] border-b border-[var(--border)]">#</th>
                {columns.map((col) => (
                  <th key={col} className="text-left px-3 py-2 font-bold text-[var(--muted-foreground)] border-b border-[var(--border)] whitespace-nowrap">{col}</th>
                ))}
              </tr>
            </thead>
            <tbody>
